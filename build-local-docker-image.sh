...
function stop_and_remove_container() {
    ...
    docker stop nocodb-private >/dev/null 2>&1
    docker rm nocodb-private >/dev/null 2>&1
}

function remove_image() {
    # Remove the existing image
    docker rmi nocodb-private >/dev/null 2>&1
}
...
function build_image() {
    # build docker
    docker build . -f Dockerfile.local -t nocodb-private --plataform  || ERROR="build_image failed"
}
...
function log_message() {
    ...
    then
        ...
        echo 'docker image with tag "nocodb-private" built sussessfully. Use below sample command to run the container'
        echo 'docker run -d -p 3333:8080 --name nocodb-private nocodb-private '
    fi
}
